...
async function readJsonInput(stream, byteLimit) {
  ...
  let text;
  try {
    text = new TextDecoder2("utf-8", { fatal: true }).decode(Buffer.concat(chunks, total));
  } catch (error) {
    throw new AgentMarkdownError("E_INPUT_INVALID", { cause: error });
  }
  try {
...
// packages/cli/dist/session-start.js
var CONTEXT_BYTES = 9e3;
var STDOUT_BYTES = 32768;
...
var SOURCES = /* @__PURE__ */ new Set(["startup", "resume", "clear", "compact"]);
function invalidInput() {
  throw new AgentMarkdownError("E_INPUT_INVALID");
}
function parseSessionStartInput(value) {
  ...
  if (typeof record.source !== "string" || !SOURCES.has(record.source))
    invalidInput();
  if (typeof record.cwd !== "string" || !isAbsoluteLocalPath(record.cwd))
    invalidInput();
...
async function reportFailure(error, io) {
...
async function sessionStartMain(io, environment2 = {}) {
  try {
    const config = await loadConfig(environment2.env === void 0 ? {} : { env: environment2.env });
    const input = parseSessionStartInput(await readJsonInput(io.stdin, config.limits.hookInputBytes));
...
// packages/cli/dist/session-start-entry.js
