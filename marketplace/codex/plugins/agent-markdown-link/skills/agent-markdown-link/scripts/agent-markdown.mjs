...
import { posix as posix4, win32 as win324 } from "node:path";
...
var STABLE_ERROR_CODE_SET = new Set(STABLE_ERROR_CODES);
var AgentMarkdownError = class extends Error {
  code;
  constructor(code, options = {}) {
    super(SAFE_MESSAGES[code], options.cause === void 0 ? void 0 : { cause: options.cause });
...
function codeFromUnknown(error) {
  try {
...
function toSanitizedDiagnostic(error) {
  const code = codeFromUnknown(error);
  return { code, message: SAFE_MESSAGES[code] };
}

// packages/core/dist/fs/safe-path.js
...
function validatePortableRelativePath(value) {
  if (value.length === 0 || CONTROL_CHARACTER.test(value) || value.includes("\\") || posix.isAbsolute(value) || win32.isAbsolute(value)) {
    unsafePath();
  }
  const segments = value.split("/");
  for (const segment of segments) {
    if (segment.length === 0 || segment === "." || segment === ".." || segment.includes(":") || /[. ]$/u.test(segment) || WINDOWS_RESERVED_NAME.test(segment)) {
      unsafePath();
    }
  }
  return value;
...
async function resolveExistingDirectory(root, relativePath) {
  const portablePath = validatePortableRelativePath(relativePath);
  const canonicalRoot = await canonicalDirectory(root);
  const canonicalTarget = await realpath(joinPortable(canonicalRoot, portablePath));
  if (!contained(canonicalRoot, canonicalTarget)) {
...
async function resolveWritableFile(root, relativePath) {
  const portablePath = validatePortableRelativePath(relativePath);
  const canonicalRoot = await canonicalDirectory(root);
...
// packages/core/dist/fs/publish.js
...
async function publishNoReplace(root, relativePath, bytes) {
  const destination = await resolveWritableFile(root, relativePath);
  const parent = path2.dirname(destination);
  const temporaryPath = path2.join(parent, `.agent-markdown-${randomUUID()}.tmp`);
  let handle;
...
// packages/core/dist/candidates/request.js
...
var SOURCE_HOSTS = /* @__PURE__ */ new Set(["codex", "claude"]);
var KINDS = /* @__PURE__ */ new Set(["decision", "fact", "preference", "project-update", "procedure", "other"]);
...
function hasDisallowedControl(value) {
  for (const character of value) {
    ...
    if (code <= 31 && code !== 9 && code !== 10 || code >= 127 && code <= 159) {
      return true;
    }
  }
  return false;
...
function credentialRule(value) {
  if (/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/u.test(value))
    return "private-key";
  if (/authorization:\s*bearer\s+\S{8,}/iu.test(value))
    return "bearer-token";
  if (/ghp_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9_]{20,}/u.test(value))
    return "github-token";
  return void 0;
}
function parseCandidateRequest(value) {
  ...
  const rationale = Object.hasOwn(candidate, "rationale") ? normalizedText(candidate.rationale) : void 0;
  const evidence = Object.hasOwn(candidate, "evidence") ? normalizedText(candidate.evidence) : void 0;
  if ([title, proposedKnowledge, rationale, evidence].some((text) => text !== void 0 && credentialRule(text))) {
...
// packages/core/dist/candidates/serialize.js
function serializeCandidate(request, metadata) {
  const frontmatter = [
    "---",
...
async function captureCandidate(config, project, request, options = {}) {
  ...
  const candidateId = (options.randomId ?? randomUUID2)();
...
// packages/core/dist/search/request.js
var SEARCH_REQUEST_BYTES = 2048;
var SEARCH_QUERY_BYTES = 1024;
var SEARCH_TERM_COUNT = 32;
var ALLOWED_FIELDS2 = /* @__PURE__ */ new Set(["schemaVersion", "query"]);
var CONTROL_CHARACTER2 = /[\u0000-\u001f\u007f-\u009f]/u;
var TERM = /[\p{L}\p{N}]+/gu;
function invalidInput2() {
  throw new AgentMarkdownError("E_INPUT_INVALID");
}
function sizeLimit() {
  throw new AgentMarkdownError("E_SIZE_LIMIT");
...
function parseSearchRequest(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value))
    invalidInput2();
  const request = value;
  if (Object.keys(request).some((key) => !ALLOWED_FIELDS2.has(key)))
    invalidInput2();
  if (request.schemaVersion !== 1 || typeof request.query !== "string")
    invalidInput2();
  if (CONTROL_CHARACTER2.test(request.query) || hasLoneSurrogate2(request.query))
    invalidInput2();
  const query = request.query.trim();
  const nonWhitespaceScalars = Array.from(query).filter((character) => !/\s/u.test(character));
  if (nonWhitespaceScalars.length < 2)
    invalidInput2();
  if (Buffer.byteLength(query, "utf8") > SEARCH_QUERY_BYTES)
    sizeLimit();
  if (extractSearchTerms(query).length > SEARCH_TERM_COUNT)
    sizeLimit();
  return { schemaVersion: 1, query };
}

// packages/core/dist/search/search.js
import { open as open3, opendir, realpath as realpath2, stat as stat2 } from "node:fs/promises";
import path3 from "node:path";
import { TextDecoder as TextDecoder2 } from "node:util";
var SEARCH_MAX_DIRECTORY_ENTRIES = 5e4;
var SEARCH_MAX_FILES = 1e4;
var SEARCH_MAX_SOURCE_BYTES = 32 * 1024 * 1024;
var SEARCH_MAX_FILE_BYTES = 64 * 1024;
var SEARCH_MAX_CONCURRENT_READS = 16;
var SEARCH_MAX_RESULTS = 8;
var SEARCH_MAX_SNIPPET_BYTES = 1024;
var SEARCH_MAX_OUTPUT_BYTES = 16 * 1024;
var SEARCH_MAX_OCCURRENCES_PER_TERM = 32;
...
function compareMatches(left, right) {
  if (left.phraseMatch !== right.phraseMatch)
    return left.phraseMatch ? -1 : 1;
  ...
  if (left.firstMatch !== right.firstMatch)
    return left.firstMatch - right.firstMatch;
  return compareOrdinal(left.relativePath, right.relativePath);
}
function escapeRegularExpression(value) {
  return value.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
}
function literalExpression(value, global = false) {
  return new RegExp(escapeRegularExpression(value), global ? "giu" : "iu");
}
function rankContent(relativePath, content, request) {
  ...
  for (const term of extractSearchTerms(request.query)) {
    ...
    for (const match of content.matchAll(expression)) {
      ...
      if (termOccurrences === SEARCH_MAX_OCCURRENCES_PER_TERM)
        break;
    }
...
function classifyFileBudget(searchedFiles, sourceBytes, fileBytes) {
  if (searchedFiles >= SEARCH_MAX_FILES)
    return "stop";
  if (fileBytes > SEARCH_MAX_FILE_BYTES)
    return "skip";
  if (sourceBytes + fileBytes > SEARCH_MAX_SOURCE_BYTES)
    return "stop";
  return "search";
}
function boundedReadCapacity(expectedFileBytes) {
  return Math.min(expectedFileBytes, SEARCH_MAX_FILE_BYTES) + 1;
}
function safeConcurrentReadWidth(searchedFiles, sourceBytes) {
  const remainingFileSlots = SEARCH_MAX_FILES - searchedFiles;
  const remainingSourceSlots = Math.floor((SEARCH_MAX_SOURCE_BYTES - sourceBytes) / SEARCH_MAX_FILE_BYTES);
  return Math.max(0, Math.min(SEARCH_MAX_CONCURRENT_READS, remainingFileSlots, remainingSourceSlots));
}
async function settleOrderedWindow(jobs) {
  if (jobs.length > SEARCH_MAX_CONCURRENT_READS)
...
async function collectSortedEntries(directory, remainingEntries) {
...
function containedRelativePath(canonicalRoot, canonicalFile) {
  const relativePath = path3.relative(canonicalRoot, canonicalFile);
  if (relativePath.length === 0 || path3.isAbsolute(relativePath) || relativePath === ".." || relativePath.startsWith(`..${path3.sep}`)) {
    throw new AgentMarkdownError("E_PATH_ESCAPE");
  }
  return validatePortableRelativePath(relativePath.split(path3.sep).join("/"));
}
...
async function readBoundedFile(filePath, expectedFileBytes) {
  ...
  if (offset > expectedFileBytes || offset > SEARCH_MAX_FILE_BYTES)
    return void 0;
  return buffer.subarray(0, offset);
}
async function inspectFile(canonicalRoot, discovered) {
  const canonicalFile = await realpath2(discovered);
  const relativePath = containedRelativePath(canonicalRoot, canonicalFile);
  const fileStatus = await stat2(canonicalFile);
  if (!fileStatus.isFile())
    throw new AgentMarkdownError("E_PATH_UNSAFE");
  return { canonicalFile, relativePath, fileBytes: fileStatus.size };
}
async function prepareFile(canonicalRoot, discovered) {
  const inspected = await inspectFile(canonicalRoot, discovered);
  const bytes = inspected.fileBytes > SEARCH_MAX_FILE_BYTES ? void 0 : await readBoundedFile(inspected.canonicalFile, inspected.fileBytes);
  return { ...inspected, bytes };
}
function addBestMatch(matches, match) {
  matches.push(match);
...
function makeSnippet(content, start, length) {
  ...
  if (matchedText !== matched.join("") || remaining === 0)
    return matchedText;
  const left = [];
  const right = [];
...
function serializedOutputBytes(response) {
...
async function searchMarkdown(config, project, request) {
  ...
  if (!(await stat2(canonicalVaultRoot)).isDirectory()) {
    throw new AgentMarkdownError("E_PATH_UNSAFE");
  }
  ...
  let truncated = false;
  let stopped = false;
  function beginFile(inspected) {
    if (seenFiles.has(inspected.canonicalFile))
      return false;
    seenFiles.add(inspected.canonicalFile);
    const initialBudget = classifyFileBudget(searchedFiles, sourceBytes, inspected.fileBytes);
  ...
  function finishFile(inspected, bytes) {
    if (bytes === void 0) {
      truncated = true;
      return;
    }
    ...
    if (match !== void 0)
      addBestMatch(matches, match);
  }
  async function processSequentialFile(discovered) {
    const inspected = await inspectFile(canonicalVaultRoot, discovered);
    if (!beginFile(inspected))
      return;
    finishFile(inspected, await readBoundedFile(inspected.canonicalFile, inspected.fileBytes));
  }
  async function walk(directory) {
    if (stopped || seenDirectories.has(directory))
      return;
    seenDirectories.add(directory);
    const batch = await collectSortedEntries(directory, SEARCH_MAX_DIRECTORY_ENTRIES - entriesVisited);
    if (batch.truncated) {
      truncated = true;
      stopped = true;
      return;
    ...
    async function flushPendingFiles() {
      ...
      while (!stopped && nextFile < discoveredFiles.length) {
        ...
        if (width === 0) {
          if (searchedFiles >= SEARCH_MAX_FILES || sourceBytes >= SEARCH_MAX_SOURCE_BYTES) {
            truncated = true;
            stopped = true;
            return;
          }
        ...
        const outcomes = await settleOrderedWindow(window.map((discovered) => () => prepareFile(canonicalVaultRoot, discovered)));
        for (const outcome of outcomes) {
          if (stopped)
            return;
          if (outcome.status === "rejected")
            throw outcome.reason;
          if (!beginFile(outcome.value))
            continue;
    ...
    for (const entry of batch.entries) {
      ...
      const discovered = path3.join(directory, entry.name);
  ...
  const results = matches.map(({ relativePath, content, snippetStart, snippetLength }) => ({
    relativePath,
    snippet: makeSnippet(content, snippetStart, snippetLength)
  }));
  let response = { schemaVersion: 1, searchedFiles, truncated, results };
...
// packages/core/dist/config/locations.js
...
// packages/core/dist/config/types.js
...
// packages/core/dist/config/locations.js
var PRODUCT_DIRECTORY = "agent-markdown-link";
...
function isAbsoluteLocalPath(value) {
  ...
  return posix2.isAbsolute(value) || win322.isAbsolute(value) && /^[A-Za-z]:[\\/]/u.test(value);
}
function normalizeAbsoluteLocalPath(value) {
  const pathApi = /^[A-Za-z]:[\\/]/u.test(value) ? win322 : posix2;
  const resolved = pathApi.resolve(value);
  const root = pathApi.parse(resolved).root;
  return resolved.length === root.length ? resolved : resolved.replace(/[\\/]+$/u, "");
}
function requireAbsoluteOverride(value) {
...
function firstAbsoluteLocal(candidates, fallback) {
  for (const candidate of candidates) {
    if (candidate !== void 0 && isAbsoluteLocalPath(candidate)) {
...
function resolveConfigPath(options = {}) {
  ...
  if (platform === "win32") {
    const appData = firstAbsoluteLocal([options.appData, env.APPDATA], win322.join(home, "AppData", "Roaming"));
    return win322.join(appData, PRODUCT_DIRECTORY, "config.json");
  }
  if (platform === "darwin") {
    return posix2.join(home, "Library", "Application Support", PRODUCT_DIRECTORY, "config.json");
  }
  const xdgConfigHome = env.XDG_CONFIG_HOME;
  const configHome = xdgConfigHome !== void 0 && posix2.isAbsolute(xdgConfigHome) && isAbsoluteLocalPath(xdgConfigHome) ? normalizeAbsoluteLocalPath(xdgConfigHome) : posix2.join(home, ".config");
  return posix2.join(configHome, PRODUCT_DIRECTORY, "config.json");
}
function resolveStateRoot(options = {}) {
  ...
  const home = getHomedir(options.homedir);
  if (platform === "win32") {
    const localAppData = firstAbsoluteLocal([options.localAppData, env.LOCALAPPDATA], win322.join(home, "AppData", "Local"));
    return win322.join(localAppData, PRODUCT_DIRECTORY);
  }
  if (platform === "darwin") {
    return posix2.join(home, "Library", "Application Support", PRODUCT_DIRECTORY, "state");
  }
  const xdgStateHome = env.XDG_STATE_HOME;
  const stateHome = xdgStateHome !== void 0 && posix2.isAbsolute(xdgStateHome) && isAbsoluteLocalPath(xdgStateHome) ? normalizeAbsoluteLocalPath(xdgStateHome) : posix2.join(home, ".local", "state");
  return posix2.join(stateHome, PRODUCT_DIRECTORY);
}
function joinStatePath(stateRoot, name) {
  return /^[A-Za-z]:[\\/]/u.test(stateRoot) ? win322.join(stateRoot, name) : posix2.join(stateRoot, name);
}
...
var config_schema_default = {
  $schema: "http://json-schema.org/draft-07/schema#",
  $id: "urn:agent-markdown-link:schema:config:v1",
  ...
  properties: {
    ...
    contextExclusions: {
      type: "array",
      items: { $ref: "#/definitions/vaultRelativePath" },
      uniqueItems: true
    },
    projects: {
      type: "array",
    ...
    obsidian: { $ref: "#/definitions/obsidian" },
  ...
  definitions: {
    ...
    limits: {
      ...
      additionalProperties: false,
      properties: {
        hookInputBytes: { type: "integer", minimum: 1, maximum: 16777216 },
        hookOutputBytes: { type: "integer", minimum: 1, maximum: 4194304 },
        contextFileBytes: { type: "integer", minimum: 1, maximum: 1048576 },
        contextTotalBytes: { type: "integer", minimum: 1, maximum: 4194304 },
        candidateBytes: { type: "integer", minimum: 1, maximum: 1048576 },
        subprocessOutputBytes: { type: "integer", minimum: 1, maximum: 16777216 },
        subprocessTimeoutMs: { type: "integer", minimum: 100, maximum: 6e4 }
      }
    },
    projectLimits: {
      type: "object",
    ...
    project: {
      type: "object",
      additionalProperties: false,
      required: ["projectId", "workspaceRoots", "contextFiles"],
      properties: {
        projectId: {
          type: "string",
          minLength: 1,
        ...
        workspaceRoots: {
          ...
          minItems: 1,
          uniqueItems: true,
          items: { $ref: "#/definitions/absolutePath" }
        },
        contextFiles: {
          type: "array",
          items: { $ref: "#/definitions/vaultRelativePath" }
        ...
        limits: { $ref: "#/definitions/projectLimits" }
      }
    },
    obsidian: {
      type: "object",
...
// packages/core/dist/config/validate.js
var DEFAULT_LIMITS = {
  hookInputBytes: 1048576,
  hookOutputBytes: 262144,
  contextFileBytes: 65536,
  contextTotalBytes: 131072,
  candidateBytes: 65536,
  subprocessOutputBytes: 262144,
  subprocessTimeoutMs: 1e4
};
var ajv = new import_ajv.default({ allErrors: true, jsonPointers: true });
var validateAgainstSchema = ajv.compile(config_schema_default);
...
function applyProjectDefaults(project, globalLimits, seenRoots) {
  ...
  for (const root of normalizedRoots) {
    ...
    if (existingProjectId !== void 0 && existingProjectId !== project.projectId) {
      throw new ConfigurationError("E_PROJECT_AMBIGUOUS", "Workspace root belongs to multiple equally specific projects.");
    }
  ...
  if (contextFileBytes > globalLimits.contextFileBytes || contextTotalBytes > globalLimits.contextTotalBytes || contextFileBytes > contextTotalBytes) {
    invalidConfig();
  }
  return {
    ...project,
    ...
    ...project.limits === void 0 ? {} : { limits: { ...project.limits } }
  };
}
...
function validateConfig(value) {
  if (hasSchemaVersion(value) && value.schemaVersion !== 1) {
    throw new ConfigurationError("E_CONFIG_VERSION", "Unsupported configuration schema version; expected version 1.");
  }
  ...
  const seenRoots = /* @__PURE__ */ new Map();
  ...
  return {
    ...
    ...config.outboxRoot === void 0 ? {} : { outboxRoot: normalizeAbsoluteLocalPath(config.outboxRoot) },
    ...
    logging: {
      maxBytes: config.logging?.maxBytes ?? 1048576,
...
// packages/core/dist/config/project.js
import { posix as posix3, win32 as win323 } from "node:path";
function pathApiFor(root) {
  return /^[A-Za-z]:[\\/]/u.test(root) ? win323 : posix3;
}
function canonicalize(value, pathApi) {
  const resolved = pathApi.resolve(value);
  const parsed = pathApi.parse(resolved);
  return resolved.length > parsed.root.length ? resolved.replace(/[\\/]+$/u, "") : resolved;
}
function comparisonValue(value, pathApi) {
  return pathApi === win323 ? value.toLocaleLowerCase("en-US") : value;
}
function contains(root, cwd, separator) {
  const descendantPrefix = root.endsWith(separator) ? root : `${root}${separator}`;
  return cwd === root || cwd.startsWith(descendantPrefix);
}
async function selectProject(config, cwd) {
  const matches = [];
  ...
  const equallySpecific = matches.filter((match) => match.comparisonRoot.length === winner.comparisonRoot.length);
  if (equallySpecific.some((match) => match.project.projectId !== winner.project.projectId)) {
    throw new ConfigurationError("E_PROJECT_AMBIGUOUS", "Working directory matches multiple equally specific projects.");
  }
  return {
    ...winner.project,
...
// packages/cli/dist/io.js
import { once } from "node:events";
import { TextDecoder as TextDecoder3 } from "node:util";
async function readJsonInput(stream, byteLimit) {
  const chunks = [];
  let total = 0;
  for await (const chunk of stream) {
    const bytes = typeof chunk === "string" ? Buffer.from(chunk, "utf8") : Buffer.isBuffer(chunk) || chunk instanceof Uint8Array ? Buffer.from(chunk) : void 0;
    if (bytes === void 0)
      throw new AgentMarkdownError("E_INPUT_INVALID");
  ...
  let text;
  try {
    text = new TextDecoder3("utf-8", { fatal: true }).decode(Buffer.concat(chunks, total));
  } catch (error) {
    throw new AgentMarkdownError("E_INPUT_INVALID", { cause: error });
  }
  try {
...
function isAbsoluteLocalPath2(value) {
  if (value.startsWith("\\\\") || value.startsWith("//"))
    return false;
  return posix4.isAbsolute(value) || win324.isAbsolute(value) && /^[A-Za-z]:[\\/]/u.test(value);
}
